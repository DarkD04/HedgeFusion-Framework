...
float4 ps_main_pm(float4 color : COLOR0, in PS_INPUT In ) : SV_TARGET
{
	...
	if(colorIndex > -1)
	{
		float4 colorA = Demultiply(Palettes.Sample(PalettesSampler, float2(colorIndex, 0.0)));
		float4 colorB = Demultiply(Palettes.Sample(PalettesSampler, float2(colorIndex, 0.5)));
		output.rgb = lerp(colorA.rgb, colorB.rgb, lerpVal);
	}
